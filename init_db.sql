...
--
--   Windows:  psql -U postgres -f init_db.sql
--   Mac/Linux: psql -U postgres -f init_db.sql
--
-- Safe to re-run: all statements use IF NOT EXISTS / ON CONFLICT DO NOTHING.
-- After running, copy .env.example to .env — no edits needed.
...
CREATE TABLE IF NOT EXISTS delivery_persons (
    ...
    vehicle_type             VARCHAR(20)  DEFAULT 'bike' CHECK (vehicle_type IN ('bike', 'cycle', 'walk', 'ev', 'scooter')),
...
-- ── 4. Indexes ─────────────────────────────────────────────────────────────
CREATE INDEX IF NOT EXISTS idx_order_cache_shop_id          ON order_cache(shop_id);
CREATE INDEX IF NOT EXISTS idx_order_cache_state            ON order_cache(state);
CREATE INDEX IF NOT EXISTS idx_order_cache_creation_time    ON order_cache(creation_time);
CREATE INDEX IF NOT EXISTS idx_order_cache_payment_method   ON order_cache(payment_method);
CREATE INDEX IF NOT EXISTS idx_order_cache_delivery_person  ON order_cache(delivery_person_id);
...
CREATE INDEX IF NOT EXISTS idx_delivery_attendance_person_date ON delivery_attendance(delivery_person_id, attendance_date);
CREATE INDEX IF NOT EXISTS idx_delivery_attendance_date     ON delivery_attendance(attendance_date);
...
-- ── 6. Seed data ───────────────────────────────────────────────────────────
-- Gives every dashboard page something to display on a fresh clone.
-- Sync vendors/orders via the UI after logging in to replace with live data.
...
INSERT INTO app_config (config_key, config_value, description) VALUES
    ...
    ('default_platform_fee_paise', '0',    'Platform fee in paise'),
...
INSERT INTO delivery_persons (name, phone, active, vehicle_type, salary_per_day_paise, per_delivery_bonus_paise, joining_date) VALUES
    ('Rahul Sharma', '9000001111', TRUE, 'bike',    50000, 500, CURRENT_DATE - 30),
    ('Suresh Kumar', '9000002222', TRUE, 'bike',    45000, 500, CURRENT_DATE - 20),
    ('Amit Singh',   '9000003333', TRUE, 'scooter', 55000, 750, CURRENT_DATE - 15)
ON CONFLICT DO NOTHING;

-- Amounts in paise (100 paise = Rs 1). Mix of COMPLETED, CANCELLED, PENDING.
...
-- Assign delivery persons to completed demo orders
